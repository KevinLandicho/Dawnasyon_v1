...
object SupabaseJavaHelper {
    ...
    // "Ma. Teresa" -> "materesa", "John-Paul" -> "johnpaul"
    private fun normalizeName(input: String): String {
        return input.lowercase()
            .replace(".", "")
            .replace(",", "")
            .replace("-", "") // Handle hyphenated names
            .replace("ñ", "n") // Handle common PH character
            .replace("\\s+".toRegex(), "") // Remove ALL spaces
            .trim()
    }

    // 2. STREET NORMALIZATION
    ...
    // ⭐ VALIDATION 1: CHECK USER (Strict Name & Email Check)
    @JvmStatic
    fun checkUserExists(fullName: String, email: String, callback: SimpleCallback) {
        CoroutineScope(Dispatchers.IO).launch {
            try {
                // 1. Check for exact EMAIL match first (Fastest)
                ...
                // 2. STRICT NAME CHECK
                // Fetch all names from 'profiles' and 'household_members'
                // Note: Fetching columns only to keep it light
                val profilesResponse = SupabaseManager.client.from("profiles")
                    .select(columns = Columns.list("full_name")).data

                val membersResponse = SupabaseManager.client.from("household_members")
                    .select(columns = Columns.list("full_name")).data
                ...
                // Compare against Profiles (Heads)
                for (item in existingProfiles) {
                    if (normalizeName(item.full_name) == inputNameNorm) {
                        duplicateFound = true
                        duplicateSource = "User/Head"
                        break
                    }
                }

                // Compare against Members (if not found yet)
                if (!duplicateFound) {
                    for (item in existingMembers) {
                        if (normalizeName(item.full_name) == inputNameNorm) {
                            duplicateFound = true
                            duplicateSource = "Household Member"
                            break
                        }
                    }
                }
                ...
                Handler(Looper.getMainLooper()).post {
                    if (duplicateFound) {
                        callback.onError("This Name is already registered as a $duplicateSource.")
                    } else {
                        callback.onSuccess()
                    }
                }
    ...
    // ⭐ VALIDATION 2: STRICT ADDRESS CHECK
    @JvmStatic
    fun checkAddressExists(houseNo: String, street: String, brgy: String, city: String, callback: SimpleCallback) {
        CoroutineScope(Dispatchers.IO).launch {
            try {
                // Fetch potential matches based on Barangay and City
                val response = SupabaseManager.client.from("profiles")
                    .select(columns = Columns.list("house_number", "street")) {
                        filter {
                            ilike("barangay", brgy)
                            ilike("city", city)
                            eq("type", "Resident")
                ...
                // Local Strict Normalization & Comparison
                ...
                for (item in existingAddresses) {
                    val dbHouseNorm = normalizeHouseNo(item.house_number)
                    val dbStreetNorm = normalizeStreet(item.street)

                    if (inputHouseNorm == dbHouseNorm && inputStreetNorm == dbStreetNorm) {
                        duplicateFound = true
                        break
                    }
                }
                ...
                Handler(Looper.getMainLooper()).post {
                    if (duplicateFound) {
                        callback.onError("This address ($houseNo $street) is already registered.")
                    } else {
                        callback.onSuccess()
                    }
                }
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                if (e.message?.contains("duplicate") == true || e.message?.contains("unique") == true) {
                    ...
                    Handler(Looper.getMainLooper()).post { callback.onError(e.message ?: "Application failed") }
                }
            }
        }
    }
    ...
    fun fetchNotifications(callback: NotificationCallback) {
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                val jsonResponse = SupabaseManager.client.from("notifications").select {
    ...
    fun fetchDonationHistory(callback: DonationHistoryCallback) {
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                val jsonResponse = SupabaseManager.client.from("donations").select(columns = io.github.jan.supabase.postgrest.query.Columns.raw("*, donation_items(*)")) {
    ...
    fun toggleLike(postId: Long, isLiked: Boolean, callback: SimpleCallback) {
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                if (isLiked) {
                    try {
                        SupabaseManager.client.from("post_likes").insert(LikeDTO(currentUser.id, postId))
                    } catch (e: Exception) {
                        // Ignore duplicate entry errors
                    }
                } else {
                    SupabaseManager.client.from("post_likes").delete {
                        filter { eq("user_id", currentUser.id); eq("post_id", postId) }
                    }
                }
                Handler(Looper.getMainLooper()).post { callback.onSuccess() }
            } catch (e: Exception) {
    ...
    fun toggleBookmark(postId: Long, isBookmarked: Boolean, callback: SimpleCallback) {
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                if (isBookmarked) {
                    try {
                        SupabaseManager.client.from("bookmarks").insert(BookmarkDTO(currentUser.id, postId))
                    } catch (e: Exception) {
                        // Ignore duplicate entry errors
                    }
                } else {
    ...
    fun updateUserProfile(
        fullName: String, contactNumber: String, province: String, city: String,
        barangay: String, street: String, avatarName: String, callback: ProfileUpdateCallback
    ) {
        CoroutineScope(Dispatchers.IO).launch {
            try {
                val currentUser = SupabaseManager.client.auth.currentUserOrNull()
...
@Serializable data class ProfileDTO(
    val id: String, val email: String, val full_name: String, val contact_number: String,
    val house_number: String, val street: String, val barangay: String, val city: String,
    val province: String, val zip_code: String, val face_embedding: String?, val type: String,
